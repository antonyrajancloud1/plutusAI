...
const NavigationBar = ({ activePage, userName, theme }) => {
  return `
    <nav class="bg-surface-800 dark:bg-dark-mode-card shadow-md sticky top-0 z-50">
        ...
        <div class="flex items-center justify-between h-16">
              ...
              return `
                <button
                  ...
                  class="nav-button flex items-center px-3 py-2 rounded-md text-sm font-medium transition-colors duration-150 ease-in-out
                    ${isActive
                      ? 'bg-primary text-surface-50 dark:bg-dark-mode-primary-accent dark:text-dark-mode-card'
                ...
                >
                  ${item.icon({ className: `h-5 w-5 mr-2 ${isActive ? 'text-surface-50 dark:text-dark-mode-card': 'text-medium-text dark:text-dark-mode-text-secondary group-hover:text-dark-mode-primary-accent'}` })}
                  ${item.name}
                </button>
              `;
            }).join('')}
          ...
          <div class="md:hidden ml-2">
            <select
              ...
              class="block w-full pl-3 pr-10 py-2 text-base border-gray-300 dark:border-dark-mode-border bg-surface-800 dark:bg-dark-mode-input-bg text-dark-text dark:text-dark-mode-text-primary focus:outline-none focus:ring-primary dark:focus:ring-dark-mode-primary-accent focus:border-primary dark:focus:border-dark-mode-primary-accent sm:text-sm rounded-md"
