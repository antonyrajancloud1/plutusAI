...
class NSEPriceAction():
    ...
    def initilize_price_action(self):
        ...
        self.mainSchedulerText="Job started for User :"+self.user_email+" Index Name: "+self.index_name+"\tnearestSupport == " + str(self.nearestSupport)+"\tnearestResistance == " + str(self.nearestResistance)
        ...
        while True:
            try:
                ...
                if int(self.IndexLTP).__round__() >= self.nearestResistance or int(self.IndexLTP).__round__() <= self.nearestSupport or int(self.IndexLTP).__round__() in self.levels:
                    ...
                    updateIndexConfiguration(user_email=self.user_email,index=self.index_name,data={'stage':'pivot_decided'})
    ...
    def CESetTargetAndStopLoss(self):
        try:
            ...
            updateIndexConfiguration(user_email=self.user_email,index=self.index_name,data={'stage':'Long :: '+str(self.IndexLTP)})
    ...
    def PESetTargetAndStoploss(self):
        try:
            self.indexValueAfterTrendDecided = self.IndexLTP + self.index_trend_check
            updateIndexConfiguration(user_email=self.user_email,index=self.index_name,data={'stage':'Short :: '+str(self.IndexLTP)})
            self.stoplossExit = int(self.indexValueAfterTrendDecided) + self.index_trend_check
            self.exitConditions.append(self.stoplossExit)
        ...
        while True:
            try:
                ...
                if not self.isCEOrderPlaced:
                    ...
                    self.placeOrder("CE")
                    addLogDetails(INFO,"Index Name: "+self.index_name+" User :"+self.user_email+"ExitConditions == " + str(self.exitConditions))
                ...
                if self.isTrailStoplossPlaced:
                    ...
                    addLogDetails(INFO,"isTrailStoplossPlaced "+str(self.currentOptionPrice)+"   "+str(self.trailedSLPrice))
                ...
                if self.IndexLTP >= float(self.indexValueWhilePlacingOrder) + self.index_trailing:
                    addLogDetails(INFO,"Index Name: "+self.index_name+" User :"+self.user_email+"Into next trailing")
                    ...
                    addLogDetails(INFO,"Index Name: "+self.index_name+" User :"+self.user_email+ "Adding new Value in exitConditions == " + str(self.exitConditions))
                if self.IndexLTP >= float(self.priceActionTarget):
                    ...
                    addLogDetails(INFO,"Index Name: "+self.index_name+" User :"+self.user_email+"into exit at sl")
                    if self.IndexLTP <= self.stoplossExit:
                        addLogDetails(INFO,"Index Name: "+self.index_name+" User :"+self.user_email+ "Considering Stoploss taking inverse trade"+str(self.stoplossExit))
                        self.exitBasedOnCondition(False,"Index Name: "+self.index_name+" Stoploss Hit - "+str(self.IndexLTP))
                        self.PESetTargetAndStoploss()
                        self.startPE()
                    else:
                        addLogDetails(INFO,"Index Name: "+self.index_name+" User :"+self.user_email+" into Trail Exit")
                        self.exitBasedOnCondition(False,"Index Name: "+self.index_name+" Trailing Exit - "+str(self.IndexLTP))
                        break
                time.sleep(self.timeInterval)
            except Exception as e:
                addLogDetails(INFO,"Index Name: "+self.index_name+" User : "+self.user_email+ " exception in startCE  -----  " + str(e))
        ...
        while True:
            try:
                ...
                if self.IndexLTP <= self.priceActionTarget:
                    ...
                    addLogDetails(INFO,"Index Name: "+self.index_name+" User :"+self.user_email+"into exit at sl")
                    if self.IndexLTP >=  self.stoplossExit:
                        addLogDetails(INFO,"Index Name: "+self.index_name+" User :"+self.user_email+"Considering Stoploss taking inverse trade")
    ...
    def exitBasedOnCondition(self,fromOptionPrice,reason):
        addLogDetails(INFO,"Index Name: "+self.index_name+" User : "+self.user_email+" Exit Based on condition called based on "+ reason)
        try:
            if self.is_demo_enabled:
                ...
                if self.isStoplossPlaced:
                    if self.currentOrderID != "":
                        addLogDetails(INFO,"Index Name: "+self.index_name+" User :"+self.user_email+" OrderId: "+self.currentOrderID)
                        self.revertOrderChecks(fromOptionPrice)
                        if self.BrokerObject.checkIfOrderExists(self.currentOrderID):
                            modify_order_details={EXCHANGE:NFO,TRADING_SYMBOL:self.currentPremiumPlaced,SYMBOL_TOKEN:self.BrokerObject.getTokenForSymbol(self.currentPremiumPlaced),ORDERID:self.currentExitOrderID,ORDER_TYPE:MARKET,QUANTITY:self.user_qty,VARIETY:NORMAL,DURATION:DAY}
                            modify_order_response= self.BrokerObject.modifyOrder(modify_order_details)
                            addLogDetails(INFO,"Index Name: "+self.index_name+" User :"+self.user_email+" ExitOrder :"+str(modify_order_response))
                            addLogDetails(INFO,"order exited successfully")
                            self.optionExitPrice=self.BrokerObject.getPrice(self.currentOrderID)
                            addLogDetails(INFO,"Index Name: "+self.index_name+" User :"+self.user_email+" Exit price = "+str(self.optionExitPrice))
                        else:
                            addLogDetails(INFO,"Index Name: "+self.index_name+" User :"+self.user_email+"current order not present")
                data={USER_ID:self.user_email,SCRIPT_NAME:self.currentPremiumPlaced,QTY:self.qty,EXIT_PRICE:self.optionExitPrice,STATUS:ORDER_EXITED}
                addOrderBookDetails(data,False)
                #self.initDefaultValues()
                addLogDetails(INFO,"Index Name: "+self.index_name+" User :"+self.user_email+" In "+self.index_name + " ExitConditions = " + str(self.exitConditions))
        except Exception as e:
    ...
    def placeStopLoss(self):
        try:
                ...
                if self.currentOptionPrice != CONNECTION_ERROR:
                    if float(self.currentOptionPrice) >= float(self.optionBuyPrice) + self.targetforsafesl:
                        if self.is_demo_enabled:
                            ...
                            addLogDetails(INFO,"Index Name: "+self.index_name+" User :"+self.user_email+" Placed Safe SL = " +str(self.trailedSLPrice))
                        else:
                            addLogDetails(INFO,"Index Name: "+self.index_name+" User :"+self.user_email+" stoplosst placed =" + str(float(self.optionBuyPrice) + self.safe_sl))
                            price=int(self.optionBuyPrice) +self.safe_sl
                            trigger_price = int(self.optionBuyPrice) +self.safe_sl
                            modify_order_details={ORDERID:self.currentOrderID,VARIETY:NORMAL,EXCHANGE:NSE,TRADING_SYMBOL:self.currentPremiumPlaced,TRANSACTION_TYPE:SELL,ORDER_TYPE:ORDER_TYPE_SL,PRICE:price,TRIGGER_PRICE:trigger_price,PRODUCT_TYPE:INTRADAY,DURATION:DAY,QUANTITY:self.user_qty}
                            self.currentOrderID = self.BrokerObject.modifyOrder(modify_order_details)
                            # currentOrderID = sell_order
                            addLogDetails(INFO,"Index Name: "+self.index_name+" User :"+self.user_email+" Sell order placed with target || Order ID = " + str(self.currentOrderID))
                        self.isTrailStoplossPlaced = True
                        slText="Index Name: "+self.index_name+" User :"+self.user_email+" Stoploss order placed: \t" + str(self.currentPremiumPlaced) + "Stoploss price: \t" + str(float(self.optionBuyPrice )+ self.safe_sl)
            ...
            addLogDetails(ERROR,"User :"+self.user_email+self.index_name+ "exception in placeStopLoss  -----  " + str(e))
        ...
        try:
            ...
            if OrderType == "CE":
                ...
                if not self.isCEOrderPlaced:
                    ...
                    addLogDetails(INFO,"Index Name: "+self.index_name+" User :"+self.user_email+" order placed already isCEOrderPlaced =" + str(self.isCEOrderPlaced))
            elif OrderType == "PE":
                ...
                if not self.isPEOrderPlaced:
                    ...
                    addLogDetails(INFO,"Index Name: "+self.index_name+" User :"+self.user_email+" order placed already isPEOrderPlaced = " + str(self.isPEOrderPlaced))
        except Exception as e:
    ...
    def revertDummyOrder(self,fromOptionPrice):
        ...
        self.optionDetails=self.BrokerObject.getCurrentPremiumDetails(NFO,self.currentPremiumPlaced)
        data={USER_ID:self.user_email,SCRIPT_NAME:self.currentPremiumPlaced,QTY:self.qty,EXIT_PRICE:self.BrokerObject.getLtpForPremium(self.optionDetails),STATUS:ORDER_EXITED}
    ...
    def placeCallOption(self):
        try:
            optionToBuy = getTradingSymbol(self.index_name) + str(self.BrokerObject.getCurrentAtm(self.index_name) - self.strike) + "CE"
            self.currentPremiumPlaced = optionToBuy
            addLogDetails(INFO,"Index Name: "+self.index_name+" User :"+self.user_email+" Current premium  = " + str(self.currentPremiumPlaced))
            buy_order_details={VARIETY:NORMAL,EXCHANGE:NFO,TRADING_SYMBOL:self.currentPremiumPlaced,SYMBOL_TOKEN:self.BrokerObject.getTokenForSymbol(self.currentPremiumPlaced),TRANSACTION_TYPE:BUY,ORDER_TYPE:MARKET,PRODUCT_TYPE:INTRADAY,DURATION:DAY,QUANTITY:self.user_qty}
            addLogDetails(INFO,"Index Name: "+self.index_name+" User :"+self.user_email+ " " + str(buy_order_details))
            
            if self.is_demo_enabled:
                order_response = self.placeDummyOrder("CE")
            else:
                order_response = self.BrokerObject.placeOrder(buy_order_details)
                addLogDetails(INFO,"Index Name: "+self.index_name+" User :"+self.user_email+ " " +str(order_response))
                if order_response['message'].__eq__('SUCCESS'):
                    ...
                    sell_order_details={VARIETY:STOPLOSS,EXCHANGE:NFO,TRADING_SYMBOL:self.currentPremiumPlaced,SYMBOL_TOKEN:self.BrokerObject.getTokenForSymbol(self.currentPremiumPlaced),TRANSACTION_TYPE:SELL,ORDER_TYPE:ORDER_TYPE_SL,PRICE:price,TRIGGER_PRICE:trigger_price,PRODUCT_TYPE:INTRADAY,DURATION:DAY,QUANTITY:self.user_qty}
                    addLogDetails(INFO,"Index Name: "+self.index_name+" User :"+self.user_email+ " " + str(sell_order_details))
                    initial_sell_order = self.BrokerObject.placeOrder(sell_order_details)
                    addLogDetails(INFO,"Index Name: "+self.index_name+" User :"+self.user_email+ " " +str(initial_sell_order))
                    self.currentOrderID = initial_sell_order["data"]["uniqueorderid"]
                    self.currentExitOrderID = initial_sell_order["data"]["orderid"]
                    self.isStoplossPlaced = True
                    self.isCEOrderPlaced = True
                    self.optionDetails = self.BrokerObject.getCurrentPremiumDetails(NFO,self.currentPremiumPlaced)
                    addLogDetails(INFO,"Index Name: "+self.index_name+" User :"+self.user_email+ " BuyOrderPlaced : \t" + self.currentPremiumPlaced + "Entry price: \t" + str(self.optionBuyPrice))
                    updateIndexConfiguration(user_email=self.user_email,index=self.index_name,data={'stage':'Long :: '+str(self.IndexLTP)})
    ...
    def placePutOption(self):
        try:
            optionToBuy = getTradingSymbol(self.index_name) + str(self.BrokerObject.getCurrentAtm(self.index_name) + self.strike) + "PE"
            self.currentPremiumPlaced = optionToBuy
            addLogDetails(INFO,"Index Name: "+self.index_name+" User :"+self.user_email+" Current premium  = " + str(self.currentPremiumPlaced))
            buy_order_details={VARIETY:NORMAL,EXCHANGE:NFO,TRADING_SYMBOL:self.currentPremiumPlaced,SYMBOL_TOKEN:self.BrokerObject.getTokenForSymbol(self.currentPremiumPlaced),TRANSACTION_TYPE:BUY,ORDER_TYPE:MARKET,PRODUCT_TYPE:INTRADAY,DURATION:DAY,QUANTITY:self.user_qty}
            addLogDetails(INFO,"Index Name: "+self.index_name+" User :"+self.user_email+ " " + str(buy_order_details))
            if self.is_demo_enabled:
                ...
                if order_response[MESSAGE].__eq__('SUCCESS'):
                    ...
                    addLogDetails(INFO,"Index Name: "+self.index_name+" User :"+self.user_email+ " BuyOrderPlaced : \t" + self.currentPremiumPlaced + "Entry price: \t" + str(self.optionBuyPrice))
                    updateIndexConfiguration(user_email=self.user_email,index=self.index_name,data={'stage':'Short :: '+str(self.IndexLTP)})
                    addLogDetails(INFO,"Index Name: "+self.index_name+" User :"+self.user_email+" short placed")
        except Exception as e:
