...
class AngelOneBroker:
        ...
        try:
            ...
            if self.user_token_data_list.__len__() > 0:
                ...
                if expiry_time > int_timestamp:
                    addLogDetails(
                        INFO,
                        "Broker object initiated with existing token " + self.user_id,
                    )
                else:
    ...
    def load_scrip_master(self):
        ...
        if not os.path.exists(self.cache_file) or not self.is_today_file(
            self.cache_file
        ):
            try:
                ...
                addLogDetails(
                    ERROR, "Failed to download or save scrip master: " + str(e)
                )
    ...
    def _handle_api_error(self, response, context):
        if isinstance(response, dict):
            ...
            if error_code == "AG8004":
                ...
                addLogDetails(
                    ERROR,
                    f"[AngelOne {error_code}] {self.user_id} - {context}: {error_msg}",
                )
        return False
    ...
    def getCurrentAtm(self, index_name):
        try:
            match index_name:
                case "nifty":
                    indexValue = 50
                    response = self.smartApi.ltpData("NSE", index_name, "99926000")
                    self._handle_api_error(response, f"getCurrentAtm({index_name})")
                    index_ltp = response["data"]["ltp"]
                    index_spot = indexValue * round(index_ltp / indexValue)
                    return index_spot
                case "bank_nifty":
                    indexValue = 100
                    response = self.smartApi.ltpData("NSE", index_name, "99926009")
                    self._handle_api_error(response, f"getCurrentAtm({index_name})")
                    index_ltp = response["data"]["ltp"]
                    index_spot = indexValue * round(index_ltp / indexValue)
                    return index_spot
                case "fin_nifty":
                    indexValue = 50
                    response = self.smartApi.ltpData("NSE", index_name, "99926037")
                    self._handle_api_error(response, f"getCurrentAtm({index_name})")
                    index_ltp = response["data"]["ltp"]
                    index_spot = indexValue * round(index_ltp / indexValue)
                    return index_spot
                case "sensex":
                    indexValue = 100
                    response = self.smartApi.ltpData("BSE", index_name, "99919000")
                    self._handle_api_error(response, f"getCurrentAtm({index_name})")
                    index_ltp = response["data"]["ltp"]
                    index_spot = indexValue * round(index_ltp / indexValue)
                    return index_spot
    ...
    def placeOrder(self, order_details):
        response = self.smartApi.placeOrderFullResponse(order_details)
        self._handle_api_error(response, "placeOrder")
        return response

    def getOrderDetails(self, unique_order_id):
        order_details = self.smartApi.individual_order_details(unique_order_id)
        if order_details:
            self._handle_api_error(order_details, f"getOrderDetails({unique_order_id})")
        addLogDetails(INFO, order_details)
        return order_details.get("data") if order_details else None
    ...
    def getLtpForPremium(self, optionDetails):
        try:
            ltpInfo = self.smartApi.ltpData(
                optionDetails[EXCHANGE],
                optionDetails[TRADING_SYMBOL],
                optionDetails[SYMBOL_TOKEN],
            )
            self._handle_api_error(
                ltpInfo, f"getLtpForPremium({optionDetails[TRADING_SYMBOL]})"
            )
    ...
    def checkIfOrderExists(self, order_details):
        try:
            response = self.smartApi.individual_order_details(order_details)
            if response:
                self._handle_api_error(response, f"checkIfOrderExists({order_details})")
            addLogDetails(INFO, response)
    ...
    def checkIfOrderPlaced(self, order_details):
        try:
            response = self.smartApi.individual_order_details(order_details)
            if response:
                self._handle_api_error(response, f"checkIfOrderPlaced({order_details})")
            addLogDetails(INFO, response)
            order_status = response["data"]["status"]
    ...
    def getCandleData(self, exchange, symbol, from_time, to_time, time_frame):
        try:
            ...
            self._handle_api_error(historic_data, f"getCandleData({symbol})")
            if historic_data.get("message") == "SUCCESS":
